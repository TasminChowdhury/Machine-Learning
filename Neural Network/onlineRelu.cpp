...
{
    srand(time(NULL));
    ofstream outfile("relu6.txt");
    int i,j,r1,r2;
    int X1[2000], X2[2000], T1[2000],T2[2000];
    int max = INT_MAX;
    int d_y1, d_y2, d_z1, d_z2;
    ...
    for (i = 0; i < 2000; i++)
    {
        ...
        u11 = u110 - e*(z1 - T1[i])*d_z1*y1;
        u21 = u210 - e*(z1 - T1[i])*d_z1*y2;
        u12 = u120 - e*(z2 - T2[i])*d_z2*y1;
        u22 = u220 - e*(z2 - T2[i])*d_z2*y2;
        w11 = w110 - e*((z1 - T1[i])*d_z1*u110 +(z2-T2[i])*d_z2*u120)*d_y1*X1[i];
        w21 = w210 - e*((z1 - T1[i])*d_z1*u110 +(z2-T2[i])*d_z2*u120)*d_y1*X2[i];
        w12 = w120 - e*((z1 - T1[i])*d_z1*u210 +(z2-T2[i])*d_z2*u220)*d_y2*X1[i];
        w22 = w220 - e*((z1 - T1[i])*d_z1*u210 +(z2-T2[i])*d_z2*u220)*d_y2*X2[i];
        b = b0 - e*(z1 - T1[i])*d_z1;
        b4 = b40 - e*(z2 - T2[i])*d_z2;
        b1 = b10 - e*((z1 - T1[i])*d_z1*u110 + (z2 - T2[i])*d_z2*u120)*d_y1;
        b2 = b20 - e*((z1 - T1[i])*d_z1*u210 + (z2 - T2[i])*d_z2*u220)*d_y2;
