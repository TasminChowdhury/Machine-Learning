...
void test(float **train_data, int *train_labels, float **test_data, int *test_labels) {
    cout << "\ntest on train data\n";
    float temp;
    float accuracy=0;
    for (int i = 0; i < 10; i++)
    {
        // FORWARD PROPAGATION
        multiply(w0_trained, train_data[i], s0_test, data_dimension, neurons_hidden_layer);
        ...
        accuracy += train_labels[i] == temp ? 1 : 0;
        //cout << train_labels[i] << " " << temp << endl;
    }
    //accuracy = ((accuracy*1.0)/10)*100;
...
/*
    ...
    {
        // FORWARD PROPAGATION
        multiply(w0_trained, test_data[i], s0_test, data_dimension, neurons_hidden_layer);
        add(s0_test, b0_trained, neurons_hidden_layer);
        activate(s0_test, y0_test, neurons_hidden_layer);
        ...
        accuracy += test_labels[i] == temp ? 1 : 0;
        //cout << test_labels[i] << " " << temp << endl;
    }
...
int main()
{
    ...
    test(train_data, train_labels, test_data, test_labels);
