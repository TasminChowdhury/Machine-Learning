	...
	for (int r = 0; r < epochs; r++) { // for one epoch
		...
		for (int k = 0; k < data_considered; k++) {
					...
					for (n = 0; n < feature_map_size; n++) {
						...
						for (i = 0; i < window_size; i++)
							for (j = 0; j < window_size; j++)
								temp += F0[l][i][j] * train_data[k][(i + m) * 24 + (j + n)]; // not 24, should be 28
			...
			for (l = 0; l < labels_dimension; l++) {
				// error signals
				delta_output[l] = (y3[l] - target[k][l]); // for delta dont we need to save the derivative also
				...
				C[l] = C0[l] - e * delta_output[l] * 1; // for bias multiplying 1
			}
			...
			for (l = 0; l < feature_map; l++) {
				...
				for (m = 0; m < downsampled_size; m++) {
					for (n = 0; n < downsampled_size; n++) {
						...
						for (i = 0; i < window_size; i++)
							for (j = 0; j < window_size; j++)
								F[l][i][j] += delta_feature_map[l][m][n] * train_data[k][(max_index_r + i) * 24 + (max_index_c + j)];// should be 28
			...
			for (l = 0; l < feature_map; l++) {
				...
				for (m = 0; m < feature_map_size; m++) {
					for (n = 0; n < feature_map_size; n++) {
						...
						for (i = 0; i < window_size; i++)
							for (j = 0; j < window_size; j++)
								temp += F0[l][i][j] * train_data[k][(i + m) * 24 + (j + n)];
			...
			for (l = 0; l < feature_map; l++) {
				...
				for (m = 0; m < feature_map_size; m++) {
					for (n = 0; n < feature_map_size; n++) {
						...
						for (i = 0; i < window_size; i++)
							for (j = 0; j < window_size; j++)
								temp += F0[l][i][j] * test_data[k][(i + m) * 24 + (j + n)];
