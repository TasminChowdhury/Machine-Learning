...
int main()
{
	ofstream outfile("filesigmoid.dat");
	int i, j, r1, r2;
	...
	for (i = 0; i < 2000; i++)
	{
		...
		y1 = 1.0 / (1.0 + exp(-(s1 + b10)));
		y2 = 1.0 / (1.0 + exp(-(s2 + b20)));
		r3 = u110 * y1 + u210 * y2;
		r4 = u120 * y1 + u220 * y2;
		z1 = 1.0 / (1.0 + exp(-(r3 + b0)));
		z2 = 1.0 / (1.0 + exp(-(r4 + b40)));
		u11 = u110 + e * z1*(z1 - T1[i])*(z1 - 1)*y1;
		u21 = u210 + e * z1*(z1 - T1[i])*(z1 - 1)*y2;
		u12 = u120 + e * z2*(z2 - T2[i])*(z2 - 1)*y1;
		u22 = u220 + e * z2*(z2 - T2[i])*(z2 - 1)*y2;
		w11 = w110 - e * ((1.0 - z1)*(z1 - T1[i])*z1*u110 + (z2 - T2[i])*(1.0 - z2)*z2*u120)*(1.0 - y1)*y1*X1[i];
		w21 = w210 - e * ((1.0 - z1)*(z1 - T1[i])*z1*u110 + (z2 - T2[i])*(1.0 - z2)*z2*u120)*(1.0 - y1)*y1*X2[i];
		w12 = w120 - e * ((1.0 - z1)*(z1 - T1[i])*z1*u210 + (z2 - T2[i])*(1.0 - z2)*z2*u220)*(1.0 - y2)*y2*X1[i];
		w22 = w220 - e * ((1.0 - z1)*(z1 - T1[i])*z1*u210 + (z2 - T2[i])*(1.0 - z2)*z2*u220)*(1.0 - y2)*y2*X2[i];
		b = b0 - e * (z1 - T1[i])*(1 - z1)*z1;
		b4 = b40 - e * (z2 - T2[i])*(1 - z2)*z2;
		b1 = b10 - e * ((z1 - T1[i])*(1.0 - z1)*z1*u110 + (z2 - T2[i])*(1.0 - z2)*z2*u120)*y1*(1.0 - y1);
		b2 = b20 - e * ((z1 - T1[i])*(1.0 - z1)*z1*u210 + (z2 - T2[i])*(1.0 - z2)*z2*u220)*y2*(1.0 - y2);
