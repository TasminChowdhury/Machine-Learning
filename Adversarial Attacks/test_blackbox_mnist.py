"""
This file attacks a PPD model trained
on MNIST dataset with adversarial examples
...
from utils import permute, mnist_denoise
...
def mnist_blackbox(train_start=0, train_end=60000, test_start=0,
  ...
  """
    ...
    seeds = range(100, 110)  # available seeds are 100, 101, ..., 150, None
    ...
    with sess.graph.as_default():
        ...
        bbox_preds = tf.py_func(lambda data: ensemble.predict(mnist_denoise(data)), [x], tf.float32)
    ...
    adv_y = tf.py_func(lambda data: ensemble.predict(mnist_denoise(data)), [x_adv_sub], tf.float32)
